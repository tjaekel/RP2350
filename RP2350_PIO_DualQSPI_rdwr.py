...
@rp2.asm_pio(fifo_join=rp2.PIO.JOIN_TX, out_shiftdir=0, pull_thresh=32, autopull=False, sideset_init=(rp2.PIO.OUT_HIGH, rp2.PIO.OUT_LOW), out_init=(rp2.PIO.OUT_LOW, rp2.PIO.OUT_LOW, rp2.PIO.OUT_LOW, rp2.PIO.OUT_LOW))
def cmdAddr():
    wrap_target()
...
def readClk():
    ...
    wait(1, irq, 4)                       #16: it waits and clears automatically
    label("clkloop")
    nop()                   [1] .side(1)  #17: QCLK starts one instruction cycle after release
    jmp(x_dec, "clkloop")   [1] .side(0)  #18: 50% duty cycle, needs 4x PIO clock
    wrap()
...
def dataRead():
    ...
    #wait(0, gpio, 15)                    #26: we wait for QCLK going low again
    wait(0, pin, 4)                       #26: GPIO11.14 = QD0..QD3, GPIO15 = SCLKin - the same as above - assume QCLK was 1 in between!
    nop()                   [2]           #27: THIS IS TRICKY!: it looks like: "wait(0, gpio, 15)" is without two clock cycles synchronizer
                                          #    but the input pins are delayed by two clocks synchronizer! Even changing - no difference!
    in_(pins, 4)            [1]           #28: get 4 bits from 4 pallel data lanes - it samples one instruction cycle after falling edge!
                                          ...
                                          #    make sure QCLK is 1 in between so that "wait(0, pin, 4)" waits for falling edge!!!
    ...
    set(pindirs, 0xf)           .side(1)  #31: set nCS high, de-assert QDIR signal = end of transfer - takes a while to see - WHY???
                                          #  : are we out of the 32 instrcutions? MAX. 32!
    wrap()

#---------------------------------------------------
#System Config and startup
    
machine.freq(150000000)                   #change from 125MHz (RP2040) to 150MHz (RP2350)

FREQ = 20000000                           #our frequency to generate (SCLK) - max. is 25MHz - 10MHz works OK
SM_NO = 0                                 #PIO2 does not work (yet)!

#configure I2C pins as GPIOs (INT0 = GPIO0, RESET = GPIO1)
INT0  = Pin(0, Pin.IN)
...
#disable clock synchronizer for AD0..QD3 (GPIO11..GPIO13) plus QCLKin - does not help really
...
def QSPIWrite(cmd:int, addr:int, alt:int, wrdata:ptr32):            #CMD: is 8bit word, ALT: is 24bit word
    CmdAddrAlt = array.array('i', [0, addr, alt << 8])
    cmd32 = int(cmd) & 0x1                           #bit  0        #CMD is special bit coded, sent as 4-lane 32bit value!
    cmd32 = cmd32 | ((int(cmd) <<  3) & 0x10)        #bit  4        #we could also set the lane 3 for nWR, e.g. high
    cmd32 = cmd32 | ((int(cmd) <<  6) & 0x100)       #bit  8
    cmd32 = cmd32 | ((int(cmd) <<  9) & 0x1000)      #bit 12
    cmd32 = cmd32 | ((int(cmd) << 12) & 0x10000)     #bit 16
    cmd32 = cmd32 | ((int(cmd) << 15) & 0x100000)    #bit 20
    cmd32 = cmd32 | ((int(cmd) << 18) & 0x1000000)   #bit 24
    cmd32 = cmd32 | ((int(cmd) << 21) & 0x10000000)  #bit 28
    CmdAddrAlt[0] = cmd32
    
    endianReverse(wrdata, len(wrdata)*4)             #change to little endian
    nCS.value(0)                                     #direct before data transfer
    sm_dma_put(0, SM_NO + 0, CmdAddrAlt, 3)
    #while sm_tx_fifo_level(SM_NO + 0) > 0:          #check if all done, we could also check PIO PC value back to 0
    #    pass
    ...
    nCS.value(1)                                     #this nCS de-asserted is way faster as on Read!
    
def QSPIRead(cmd:int, addr:int, alt:int, rddata:ptr32):
    CmdAddrAlt = array.array('i', [0, addr, alt << 8])
    cmd32 = int(cmd) & 0x1                           #bit  0
    cmd32 = cmd32 | ((int(cmd) <<  3) & 0x10)        #bit  4
    cmd32 = cmd32 | ((int(cmd) <<  6) & 0x100)       #bit  8
    cmd32 = cmd32 | ((int(cmd) <<  9) & 0x1000)      #bit 12
    cmd32 = cmd32 | ((int(cmd) << 12) & 0x10000)     #bit 16
    cmd32 = cmd32 | ((int(cmd) << 15) & 0x100000)    #bit 20
    ...
    Num2Rd = len(rddata)                             #up to 5 words are read in a single burst, later with gaps
    ...
    for i in range(Num2Rd):
        ...
        rddata[i] = r
    ##sm_dma_get(0, SM_NO + 3, rdData, Num2Rd)       #DMA GET DOES NOT WORK!!!! Why?
    nCS.value(1)                                     #WHY does it take so much time to de-assert nCS????
    endianReverse(rddata, Num2Rd*4)                  #back to little endian
    
while True:
    #-- WRITE --:
    wrData = array.array('i', [0x2468ABCD, 0x12345678, 0x98765432, 0xFEDCBA98])
    QSPIWrite(0x57, 0x12345678, 0xabcdef, wrData)
    
    #RESET.value(0) #just for testing signal
    
    #-- READ --:
    rdData = array.array('i', [0, 0, 0, 0, 0])
    QSPIRead(0x75, 0x87654321, 0xfedcbf, rdData)
    print(rdData)
    
    #RESET.value(1) #just for testing
