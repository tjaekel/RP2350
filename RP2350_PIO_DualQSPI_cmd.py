...
# GP7 : nCS  - SW control
# GP8 : nCS2 - SW control
# GP9 : QDIR signal (for external level shifter with DIR signal), 1 = READ, 0 = WRITE
# GP10: SCLK
# GP11: DIO0
# GP12: DIO1
# GP13: DIO2
# GP14: DIO3
# GP15: QCLKin - feedback clock - directly used as pin (connect GP10 (QCLK) with GP15 (QCLKfb)
# Remark:
...
#
# using DMAs: we have total 16 DMA channels - Internet 12 DMA channels
# ATTENTION: make sure to use a free DMA (e.g. not one used internally by MicroPython)!
#
# There is a MISSING FEATURE and an issue:
# a) we can check for "DMA done" - but it does not mean all was shifted out!
#    There are Tx FIFOs and "DMA done" means just 'it could write to Tx FIFO' but not
#    yet shifted out all! So, we use 'Tx FIFO empty' to avoid
# b) but 'Tx FIFO empty' does not mean that OSR was completely shifted out!
#    on slower clocks to send still possible to override the SM!
# ==> there is not a PIO SM status check possible if all was shifted out!
# we have always a minimum speed where it does not work anymore below this frequency (2 MHz)
# we cannot SYNC with SM (by the main program): even using the SM ADDR is not working in order
# to symchronize
# strange is: the ADDR (as 'current' instruction) seems to be +1, but the SMs seem to use the
# instruction memory in a decending order (SM0 is on last instruction memory, filling all backwards)
#+++++++++++++++++++++++++++++++++++++++++++++++++
...
def sm_get_addr(sm: int) -> int:
    ...
    sm %= 4
    smx = SM_REG_BASE + sm * SMx_SIZE + SMx_ADDR
    return (pio[smx])

@micropython.viper
def sm_get_instr(sm: int) -> int:
    if sm < 4:   # PIO 0
        pio = ptr32(uint(PIO0_BASE))
    elif sm < 8: # PIO 1
        pio = ptr32(uint(PIO1_BASE))
...
#
# Get the current DMA busy
#
#@micropython.viper
#def dma_busy(chan:uint) -> bool:
#    dma=ptr32(uint(DMA_BASE) + chan * 0x40)
#    if dma[CTRL_TRIG] & BUSY:
#        return True
#    else:
#        return False
    
@micropython.viper
def dma_busy(chan:uint) -> int:
    dma=ptr32(uint(DMA_BASE) + chan * 0x40)
    return dma[CTRL_TRIG] & BUSY
...
def dma_transfer_count(chan:uint) -> int:
    dma=ptr32(uint(DMA_BASE) + chan * 0x40)
    return dma[TRANS_COUNT] & 0x0FFFFFFF
#
...
#GPIO pin of                                                               bit0 = QDIR,      bit1 = QCLK,                bit0..3 = DATA (out, 4 data lanes)
...
#                                                                           bit0: QCLK     autopush, autopoll not needed here
@rp2.asm_pio(in_shiftdir=0, pull_thresh=32, push_thresh=32, sideset_init=(rp2.PIO.OUT_HIGH))
def readClk():
    wrap_target()
...
#                                                              True fails!   push() needed?                   bit0: QDIR, bit1: QCLK
...
def dataRead():
    ...
    mov(y, osr)             [2] .side(1)  #20: keep number of words to read, QCLK=0, QDIR=1 already - here as earliest as possible
    set(pindirs, 0x0)       [2] .side(3)  #21: change direction, QCLK=1, QDIR=1
    nop()                   [2] .side(1)  #22: QCLK=0, QDIR=1
    ...
    in_(pins, 4)                          #28: get 4 bits from 4 pallel data lanes - it samples one instruction cycle after falling edge!
    ...
    #push(noblock)                        #  : we use autopush to save number of instructions
...
FREQ = 2000000                            #our frequency to generate (SCLK) - max. is 25MHz - 10MHz works OK
#minimal FREQ: 2MHz - below, e.g. 500KHz, main code overrides the SM! (not sync with shift out completion)
#the SW CSn is too early!
SM_NO = 0                                 #0,4,8: PIO2(=8) does not work (yet)!
...
#RE signal:
#RE = Pin(16, Pin.OUT, value=0)            #default: a WRITE = 0
...
sm3 = rp2.StateMachine(SM_NO + 3, dataRead, freq=6*FREQ, sideset_base=Pin(9), in_base=Pin(11), set_base=Pin(11))
sm3.active(1)
...
def QSPI_Write(valArr):
    ...
    cmd |= (cmdVal & 0x80) << 21 
    alt = valArr[2] << 8;
    CmdAddrAlt = array.array('i', [cmd, valArr[1], alt])
    WrData = array.array('i', [])
    for i in range(3, len(valArr)):
        WrData.append(valArr[i])
    ...
    endianReverse(WrData, len(WrData)*4)
    nCS.value(0)                 #direct before data transfer
    #RE.value(0)                  #announce a WRITE cycle

    #with DMA: way faster burst              
    sm_dma_put(0, SM_NO + 0, CmdAddrAlt, 3)
    while sm_tx_fifo_level(SM_NO + 0) > 0:
        pass
    #while (dma_busy(0)):                   #we cannot use "DMA done"! It does not mean the PIO FIFO was shifted out!
    #    pass
    #while sm_get_addr(SM_NO + 0) != 24:    #wait for SM back to wait - fails in the same way on slower speed
    #    pass
    
    if len(WrData) > 0:
        sm_dma_put(0, SM_NO + 1, WrData, len(WrData))
        while sm_tx_fifo_level(SM_NO + 1) > 0:
            pass
        #while (dma_busy(0)):                   #we cannot use "DMA done"! It does not mean the PIO FIFO was shifted out!
        #    pass
        #while sm_get_addr(SM_NO + 1) != 19:
        #    pass
    
    nCS.value(1)

def QSPI_Read(valArr, rdWords):
    cmdVal = valArr[0];
    cmd  = cmdVal  &  0x1
    cmd |= (cmdVal &  0x2) << 3
    cmd |= (cmdVal &  0x4) << 6
    ...
    RdData = array.array('i', [0] * rdWords)
    ...
    #while (dma_busy(0)):                #we cannot use "DMA done"! It does not mean the PIO FIFO was shifted out!
    #    pass
    #while sm_get_addr(SM_NO + 0) != 24:
    #    pass
    
    #we have to enable DMA before releasing READ part!
    sm_dma_get(0, 3, RdData, rdWords)         #can we see any DMA error code
    sm3.put(rdWords - 1)                       #write the number of words -1
    #while sm_rx_fifo_level(SM_NO + 3) > 0:
    #    pass
    while (dma_busy(0)):                      #this is OK: Rx FIFO should be empty
        pass
    nCS.value(1)
    #RE.value(0)

    endianReverse(RdData, rdWords*4)
    for value in RdData:
        value &= 0xFFFFFFFF
...
def cid():
    ...
    Num2Wr = 3
    sm_dma_put(0, SM_NO + 1, WrData, Num2Wr)
    while sm_tx_fifo_level(SM_NO + 1) > 0:
        pass
    #while (dma_busy(0)):                   #we cannot use "DMA done"! It does not mean the PIO FIFO was shifted out!
    #    pass
    ...
    while sm_tx_fifo_level(SM_NO + 0) > 0:
        pass
    #while (dma_busy(0)):     #RE.value(0)                #we cannot use "DMA done"! It does not mean the PIO FIFO was shifted out!
    #    pass
    #while sm_get_addr(SM_NO + 0) != 24:
    #    pass
    ...
    chipID = RdData[3] & 0xFFFFFFFF
    print(f"0x{chipID:08X}")
        
def led():
    #-- WRITE --:
    
    CmdAddrAlt = array.array('i', [0x11100000, 0x00000000, 0x00001F00])
    WrData = array.array('i', [0x00040180, 0x400B0FA0, 0xAED00000, 0x00000000, 0xE0120005, 0x1FEDFFFA])
    Num2Wr = 6
    Num2Rd = 3
    endianReverse(WrData, 6*4)
    nCS.value(0)                 #direct before data transfer
    #RE.value(0)                  #announce a WRITE cycle
    ...
    #                                          #make sure to be fast for read value!
    #for i in range(Num2Rd):                   #after 5 words we get gaps
    #    r = sm3.get()                         #the same issue here: the byte order is "inversed"! flip it back to LITTLE ENDIAN
    #                                          #if too fast QCLK - we can hang here!
    #    RdData[i] = r
          
    ##DMA channel > 0 DOES NOT WORK YET! - something is sent afterwards - e.g. when we use DMA channel 1!
    #----------------------------------------------------------------------------------------------------
        
    #we have to enable DMA before releasing READ part!
    sm_dma_get(0, SM_NO + 3, RdData, Num2Rd)
    sm3.put(Num2Rd - 1)                      #write the number of words -1
    #while sm_rx_fifo_level(SM_NO + 3) > 0:
    #    pass
    while (dma_busy(0)):                     #this is OK, Rx FIFO should be empty
...
#-------------------------------------------------------------------
# command decoder:
# wait for am ASCII command line, decode and call the QSPI functions
# convert the parameters given into arguments on function calls
#

def print_help():
    print("help     : print this help text")
    print("cid      : read and display chipid")
    print("led      : switch on test LED")
    print("qspi     : qspi cmd addr alt [word ...] - WRITE transaction")
    print("qspi -N  : qspi -rdWords cmd addr alt - READ transaction")
    print("quit     : end endless command loop, back to Python shell")
...
while True:
    ...
    if cmdList[0] == 'qspi':
        #convert the string parameters into values
        if int(cmdList[1]) < 0:
            optArg = int(cmdList[1]) *-1
            l = len(cmdList) - 2    #w/o command word and -opt
            off = 2
        else:
            l = len(cmdList) - 1    #w/o command word
            off = 1
            
        args = array.array('i', [])
        for i in range(l):
            args.append(int(cmdList[off + i]))
